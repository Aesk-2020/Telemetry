/*
 * Can_Lyra_Header.h
 *
 *  Created on: 8 Nis 2020
 *      Author: ahmet
 */

#ifndef CAN_LYRA_HEADER_H_
#define CAN_LYRA_HEADER_H_
#include "main.h"

//MCU
#define ACTUAL_VALUES						0x1555DDD0
#define SET_VALUES							0x1555DDD1
#define	ERROR_STATUS						0x1555DDD2
#define WAKE_UP_COMMANDS					0x15550000

//VCU
#define WAKE_UP_COMMANDS					0x15550000
#define SEND_PI_ID							0x15550011
#define SEND_D_ID							0x15550012
#define QUERY_ID							0x15550013
#define QUERY_RESPONSE_PI					0x15550014
#define QUERY_RESPONSE_D					0x15550015

//EMS
#define EMS_CURRENT            				0x1555EEE0
#define EMS_VOLTAGE							0x1555EEE1
#define EMS_CONSUMPTION						0x1555EEE2
#define EMS_STATE_DATA						0x1555EEE3

//BMS
#define BMS_MEASUREMENTS					0x1555BBB0
#define BMS_STATE_DATA						0x1555BBB1
#define BMS_CELLS_1							0x1555BBB2
#define BMS_CELLS_2							0x1555BBB3
#define BMS_CELLS_3							0x1555BBB4
#define BMS_CELLS_4							0x1555BBB5
#define BMS_SOC_1							0x1555BBB6
#define BMS_SOC_2							0x1555BBB7
#define BMS_SOC_3							0x1555BBB8
#define BMS_SOC_4							0x1555BBB9


/*
typedef union
{
	struct
	{
		uint8_t ZPC_Completed 		  : 1;
		uint8_t PWM_Enabled	  		  : 1;
		uint8_t Dc_Bara_Error 		  : 1;
		uint8_t Temp_Error	  		  : 1;
		uint8_t Dc_Bara_Current_Error : 1;
		uint8_t ID_Error			  : 1;
		uint8_t Wake_Up_Control       : 1;
		uint8_t Reserved			  : 1;
	}Driver_error;
	uint8_t driver_error_u8;
}Driver_error_union;
Driver_error_union driver_states;

typedef union
{
	struct
	{
		uint8_t Direction : 1;
		uint8_t Brake	  : 1;
		uint8_t Ignition  : 1;
	}Drive_status;
	uint8_t drive_status_u8;
}Drive_status_union;



typedef struct
{
	float Phase_A_Current_f32;
	float Phase_B_Current_f32;
	float Dc_Bus_Current_f32;
	float Dc_Bus_voltage_f32;
	float Id_f32;
	float Iq_f32;
	float IArms_f32;
	float Torque_f32;
	uint32_t Odometer_u32;
	uint8_t Motor_Temperature_u8;
	uint8_t actual_velocity_u8;
	Driver_error_union driver_error_union;
	Drive_status_union drive_status_union;
}Driver_Datas;
*/

typedef struct
{
	uint8_t ID_Act_L;
	uint8_t ID_Act_H;
	uint8_t IQ_Act_L;
	uint8_t IQ_Act_H;
	uint8_t VD_Act_L;
	uint8_t VD_Act_H;
	uint8_t VQ_Act_L;
	uint8_t VQ_Act_H;
	uint8_t ID_Set_L;
	uint8_t ID_Set_H;
	uint8_t IQ_Set_L;
	uint8_t IQ_Set_H;
	uint8_t set_torque_L;
	uint8_t set_torque_H;
	uint8_t IDC_L;
	uint8_t IDC_H;
	uint8_t VDC_L;
	uint8_t VDC_H;
	uint8_t act_speed_L;
	uint8_t act_speed_H;
	uint8_t motor_temp;
	uint8_t error_status_L;
	uint8_t error_status_H;
	uint8_t act_torque;
}Driver_Data;


/*
 * 	Mode bits
 *  00 => No operation
 *  01 => Free wheeling (Ignition off)
 *  10 => Speed Control
 *  11 => Torque Control
 *
 *  Brake
 *  1 => On
 *  0 => Off
 */
typedef union
{
	struct
	{
		uint8_t BMS_WUP		: 1;
		uint8_t MCU_WUP		: 1;
		uint8_t Freewheeling: 1;
		uint8_t Mode	  	: 1;
		uint8_t Brake  		: 1;
		uint8_t reserved  	: 3;
	}Drive_status;
	uint8_t drive_command_u8;
}Drive_command_union;

typedef struct
{
	Drive_command_union drive_command_union;
	uint16_t 			speed_set_rpm_u16;
	int16_t 			set_torque_s16;
	uint16_t 			speed_limit_u16;
	uint8_t 			torque_limit_u8;
}VCU_Datas;


typedef union
{
	struct
	{
		uint8_t High_Voltage_Error   : 1;
		uint8_t Low_Voltage_Error    : 1;
		uint8_t Temp_Error           : 1;
		uint8_t Comminication_Errror : 1;
		uint8_t Over_Current_Error   : 1;
		uint8_t Fatal_Error			 		 : 1;
		uint8_t Reserved             : 2;
	}State_Data_Error;
	uint8_t bms_error_u8;
}Bms_State_Union;

typedef union
{
	struct
	{
		uint8_t Precharge_Flag : 1;
		uint8_t Discharge_Flag : 1;
		uint8_t Dc_Bus_Ready   : 1;
		uint8_t Charging			 : 1;
		uint8_t Reserved       : 4;
	}Bms_State_Data_Dc_Bus_State;
	uint8_t dc_bus_state_u8;
}Dc_Bus_State_union;

typedef struct
{
	uint8_t Cell_1_u8;
	uint8_t Cell_2_u8;
	uint8_t Cell_3_u8;
	uint8_t Cell_4_u8;
	uint8_t Cell_5_u8;
	uint8_t Cell_6_u8;
	uint8_t Cell_7_u8;
	uint8_t Cell_8_u8;
	uint8_t Cell_9_u8;
	uint8_t Cell_10_u8;
	uint8_t Cell_11_u8;
	uint8_t Cell_12_u8;
	uint8_t Cell_13_u8;
	uint8_t Cell_14_u8;
	uint8_t Cell_15_u8;
	uint8_t Cell_16_u8;
	uint8_t Cell_17_u8;
	uint8_t Cell_18_u8;
	uint8_t Cell_19_u8;
	uint8_t Cell_20_u8;
	uint8_t Cell_21_u8;
	uint8_t Cell_22_u8;
	uint8_t Cell_23_u8;
	uint8_t Cell_24_u8;
	uint8_t Cell_25_u8;
	uint8_t Cell_26_u8;
	uint8_t Cell_27_u8;
	uint8_t Cell_28_u8;
}Bms_Cells;

typedef struct
{
	int8_t offset_SoC_1_u8;
	int8_t offset_SoC_2_u8;
	int8_t offset_SoC_3_u8;
	int8_t offset_SoC_4_u8;
	int8_t offset_SoC_5_u8;
	int8_t offset_SoC_6_u8;
	int8_t offset_SoC_7_u8;
	int8_t offset_SoC_8_u8;
	int8_t offset_SoC_9_u8;
	int8_t offset_SoC_10_u8;
	int8_t offset_SoC_11_u8;
	int8_t offset_SoC_12_u8;
	int8_t offset_SoC_13_u8;
	int8_t offset_SoC_14_u8;
	int8_t offset_SoC_15_u8;
	int8_t offset_SoC_16_u8;
	int8_t offset_SoC_17_u8;
	int8_t offset_SoC_18_u8;
	int8_t offset_SoC_19_u8;
	int8_t offset_SoC_20_u8;
	int8_t offset_SoC_21_u8;
	int8_t offset_SoC_22_u8;
	int8_t offset_SoC_23_u8;
	int8_t offset_SoC_24_u8;
	int8_t offset_SoC_25_u8;
	int8_t offset_SoC_26_u8;
	int8_t offset_SoC_27_u8;
	int8_t offset_SoC_28_u8;
}Bms_SocS;

typedef struct
{
	uint8_t temp_1_u8;
	uint8_t temp_2_u8;
	uint8_t temp_3_u8;
	uint8_t temp_4_u8;
	uint8_t temp_5_u8;
	uint8_t temp_6_u8;
	uint8_t temp_7_u8;

}Bms_Temps;

typedef struct
{
	float    Bat_Voltage_f32;
	float    Bat_Current_f32;
	float    Bat_Cons_f32;
	float    Soc_f32;
	float    Worst_Cell_Voltage_f32;
	uint8_t  Worst_Cell_Address_u8;
	uint8_t  Temperature_u8;
	Bms_Temps bms_temps;
	Bms_Cells	bms_cells;
	Bms_State_Union bms_error;
	Dc_Bus_State_union dc_bus_state;
	Bms_SocS bms_soc;
}Bms_Datas;

typedef union
{
	struct
	{
		uint8_t vcu_can_error 		 : 1;
		uint8_t bms_can_error 		 : 1;
		uint8_t driver_can_error   : 1;
		uint8_t Reserved       		 : 5;
	}can_error_state;
	uint8_t can_error_u8;
}Can_error_union;

typedef struct
{
	Bms_Datas 	 bms_data;
	VCU_Datas 	 vcu_data;
	Driver_Data  driver_data;
	uint32_t vcu_can_ID_1_counter;
	uint32_t bms_can_ID_1_counter;
	uint32_t bms_can_ID_2_counter;
	uint32_t bms_can_ID_3_counter;
	uint32_t bms_can_ID_4_counter;
	uint32_t bms_can_ID_5_counter;
	uint32_t bms_can_ID_6_counter;
	uint32_t driver_can_ID_1_counter;
	uint32_t driver_can_ID_2_counter;
	uint32_t driver_can_ID_3_counter;
	Can_error_union can_error;
}LyraDatas;
#endif /* CAN_LYRA_HEADER_H_ */
