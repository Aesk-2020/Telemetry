...
{
    public delegate void DataIteration();
    public static class DataFlow
    {
        #region Data_Converter_Functions
        ...
        //DataUpdated Eventine frontlar kendi update fonksiyonları ile abone olacaklar
        //DataFlow.DataUpdated += Update(); //gibi
        //UpdateFrontu mqtt eventi içinde vs çağıracağız
        //flutter provider mantığı gibi

        private static Thread _flowThread;


        public static event DataIteration DataUpdated;

        public static void Mqtt_Packet_Decomposer(byte[] mqtt_message)
        {

        }
        public static void UpdateFront()
        {
            new Thread(() => { DataUpdated.Invoke(); }).Start();
        }
    }
}
